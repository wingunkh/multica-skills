...
- 보존할 대상은 정보이지 표현이 아니다. 같은 정보를 다른 표현으로 두 번 쓰는 것은 보존이 아니라 중복이므로, 합치거나 한 번만 쓴다.
...
- 날짜 줄과 첫 레이블 사이, 레이블 블록과 다음 레이블 블록 사이에는 반드시 `빈 줄 → --- → 빈 줄` 세 줄을 넣는다. `---`는 화면에 구분선으로 그려져 블록이 눈에 띄게 나뉜다.
- `---`는 백틱으로 감싸지 않고 줄 첫 칸에 대시 세 개만 쓴다. 앞뒤 빈 줄을 빠뜨리면 바로 위 줄이 제목으로 커지므로 반드시 빈 줄을 둔다.
...
1. **형식**: 첫 두 줄이 `제목:`, `처리 상태:`이고 그다음 빈 줄, 날짜 줄 순서인가
2. **간격**: 날짜 줄과 첫 레이블 사이, 각 레이블 블록 사이에 `빈 줄 → --- → 빈 줄`이 있고, 레이블과 불릿 사이·불릿과 불릿 사이에는 빈 줄이 없으며, 모든 줄이 공백 없이 `■`, `- `, `[`, `---` 중 하나로 시작하는가
3. **레이블**: 순서가 맞고 중복·빈 레이블이 없는가
4. **식별자**: 원문의 IP, 경로, 계정, 도메인, 제품명, 파일명, 설정값이 모두 남아 있고, 경로·파일명·명령어·설정값·에러 메시지·마스킹 값이 백틱 안에 있는가
5. **마스킹**: 비밀번호·토큰 값이 `******`로 바뀌었는가
6. **정제**: 문장 어미("~습니다"), 인사말, 메일 헤더가 없고 본문에 기관 이름이 노출되지 않았는가
7. **사실성**: 원문에 없는 원인, 조치, 후속이 없고, 조치마다 무엇을 안내·전달했는지 적혀 있는가
8. **중복**: 같은 정보가 두 번 들어가지 않았는가. 레이블 사이(확인·조치·후속, 요청·후속), 같은 레이블 안, 합친 불릿 안을 모두 본다. 같은 경로나 같은 시각을 다루는 항목은 한 불릿인가
9. **배치**: 자사가 수행한 문의·테스트·조회는 조치에, 벤더·솔루션사·기관 측이 수행한 작업은 확인에 있는가
10. **요청**: 요청 한 건이 불릿 한 줄이고, 조건부 요청의 조건이 남아 있으며, 벤더·솔루션사 요청에는 `업체명 측 :` 주체가 붙었는가
11. **후속**: 요청을 다시 풀어 쓰지 않고 남은 행위만 있으며, 자사가 아닌 주체의 일과 향후 계획에는 주체가 붙었는가
12. **날짜**: 기준일이 아닌 날짜가 섞인 레이블은 모든 항목에 `(MM/DD)`가 붙었고, 날짜가 모두 기준일인 레이블과 요약·후속에는 붙지 않았는가
13. **처리 상태**: 후속의 주체와 맞는가. 후속에 주체 없는 자사 몫이 있는데 이관으로 판단하지 않았는가
...
■ 후속
- 솔루션사 측 8/31 14:43 전후 애플리케이션(변환/뷰어) 로그 확인 필요
